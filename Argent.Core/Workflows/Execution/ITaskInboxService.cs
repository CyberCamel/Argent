...
public interface ITaskInboxService
{
    ...
    Task<TaskListPage> QueryForUserAsync(
        string userId,
        List<string> roles,
        TaskListRequest request,
        CancellationToken ct = default);

    Task ClaimAsync(Guid taskId, string userId, List<string> roles, CancellationToken ct = default);

    Task ReleaseAsync(Guid taskId, string userId, CancellationToken ct = default);
