...
(function () {
    document.addEventListener('pointerdown', (e) => {
        const resizer = e.target.closest('.modeler-resizer[data-resize-prop]');
        if (!resizer) return;

        const layout = resizer.closest('.modeler-layout');
        if (!layout) return;
...
window.Modeler = {
    ...
    syncClocks() {
        ...
        let style = document.getElementById('modeler-clock-sync');
        if (!style) {
            style = document.createElement('style');
            style.id = 'modeler-clock-sync';
            document.head.appendChild(style);
        }
        style.textContent =
            `.modeler-canvas .clock-hand-hour   { animation-delay: -${hDel}s; }` +
            `.modeler-canvas .clock-hand-minute { animation-delay: -${mDel}s; }` +
            `.modeler-canvas .clock-hand-second { animation-delay: -${sDel}s; }`;
    }
};
