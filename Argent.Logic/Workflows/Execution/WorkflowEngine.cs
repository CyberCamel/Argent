...
public class WorkflowEngine(
    ILogger<WorkflowEngine> _logger,
    IServiceProvider _serviceProvider) // Inject the provider, not the scoped services
    : BackgroundService
{
    ...
    private SemaphoreSlim _semaphore = new(50);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogInformation("Executing workflow tick...");
            ...
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                foreach (var workItem in availableWork)
                {
                    ...
                    {
                        _logger.LogWarning("Max concurrency reached. Skipping WorkItem {Id} for now.", workItem.Id);
                        continue; // Skip this item for now, it will be retried in the next tick
                    }
                    // 4. Try to claim the lock (Atomic operation)
                    if (await workRepository.TryLockWorkItemAsync(workItem.Id))
                    {
                        try
                        {
                            _logger.LogInformation("Claimed WorkItem {Id}. Dispatching...", workItem.Id);
