...
public class PolicyDecisionServiceTests
{
    ...
    public async Task Group_based_policy_matches()
    {
        ...
        var svc = CreateService(ctx =>
        {
            ctx.GroupMemberships.Add(new Argent.Core.Identity.GroupMembership { GroupId = groupId, UserId = userId });
            ctx.PolicyDocuments.Add(new PolicyDocument
            {
                Name = "Group access",
                Effect = PolicyEffect.Allow,
    ...
    public async Task Nested_group_membership_is_transitive()
    {
        ...
        var svc = CreateService(ctx =>
        {
            ...
            ctx.GroupMemberships.Add(new Argent.Core.Identity.GroupMembership { GroupId = childGroup, UserId = userId });
            ctx.GroupGroupMemberships.Add(new Argent.Core.Identity.GroupGroupMembership { GroupId = parentGroup, MemberGroupId = childGroup });
            ctx.PolicyDocuments.Add(new PolicyDocument
            {
