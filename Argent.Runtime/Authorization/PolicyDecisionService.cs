...
public class PolicyDecisionService : IPolicyDecisionService
{
    ...
    private readonly ConcurrentDictionary<string, CacheEntry<List<PolicyDocument>>> _cache = new();
    private readonly ConcurrentDictionary<string, CacheEntry<List<string>>> _groupCache = new();
        ...
        CancellationToken ct = default)
    {
        ...
        _cache[cacheKey] = new CacheEntry<List<PolicyDocument>>(applicable, DateTime.UtcNow);
        return applicable;
    }
    ...
    {
        if (_groupCache.TryGetValue(userId, out var cached) && !cached.IsExpired)
            return cached.Value;
        ...
        _groupCache[userId] = new CacheEntry<List<string>>(groups, DateTime.UtcNow);
        return groups;
    }
