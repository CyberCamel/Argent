...
Workflow start forms and task forms reuse `FormRuntimeService`. A successful start-form submission creates a record, then starts a workflow instance tied to that record. The instance overview and detail pages read execution state through `IWorkflowInstanceViewStore`.
